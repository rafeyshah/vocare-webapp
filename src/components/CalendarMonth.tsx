        ...
        <div className="flex flex-col h-screen">
            <div className="flex flex-col lg:flex-row flex-1">
                <div className="flex-[3] min-w-0 px-2 py-4 bg-white">
                    {/* Day Names */}
                    <div className="hidden lg:grid border-t border-b grid-cols-7 text-xs text-center text-gray-500 uppercase">
                        {daysOfWeek.map((day) => (
                    ...
                    {/* Calendar Grid */}
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 text-xs text-gray-800">
                        ...
                        {Array.from({ length: totalDays }).map((_, i) => {
                            ...
                            return (
                                <div
                                    key={day}
                                    className="relative border-b border-r p-1 h-[110px] bg-white overflow-y-auto"
                                >
                                    <div className="block lg:hidden text-[10px] text-gray-500 font-medium mb-0.5">
                                        {daysOfWeek[weekdayIndex]}
                                    </div>
                                    <div className={`text-xs font-semibold ${isToday ? 'text-white bg-gray-900' : 'text-gray-800'} px-1 py-0.5 rounded w-fit`}>
                                    ...
                                    {dayAppointments.map((appt, idx) => (
                                        <div
                                            key={idx}
                                            className="mt-2 sm:mt-1 rounded-md p-1 bg-white border-l-4 shadow-sm text-[10px] whitespace-normal break-words"
                ...
                <div className="flex-[1] min-w-[280px] overflow-y-auto px-4 py-2 border-t lg:border-t-0 lg:border-l bg-gray-50">
